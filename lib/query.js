...
Query.prototype.handleDelta = function(delta) {
  if (delta.op === 'update') {
    // we need to see if the record is already in this result set
    // and whether it now passes the whereFn
    //this.emit('delta', delta);
    console.log("XXXX", delta);
    var isIn = this.pkPresent[delta.pk];
    var shouldBeIn = this.whereFn(delta.row);
    if (isIn && !shouldBeIn) { // delete
      var newDelta = { op: 'delete', pk: delta.pk };
      this.emit('delta', newDelta);      
    } else if (!isIn && shouldBeIn) { // add
      var newDelta = { op: 'insert', row: delta.row };
      this.emit('delta', newDelta);      
